...
specgram_rev <- function(x, n = min(256, length(x)), Fs = 2, window = hanning(n),
  ...
  res <- list(S = S, f = f, t = t, af = af)
  class(res) <- "specgram"
  res
}


print.specgram <- plot.specgram <- function(x, col=gray.colors(128,start=0,end=1,rev=T),
                                            zlim=c(20,60),
                                            xlab="Time (msec)", logsc=TRUE,
                                            ylab="Frequency (Hz)", ...) {
  if (logsc) {
    a <- 20.0 * log10(x$af * abs(x$S) + 0.001)
  } else {
    a <- x$af * abs(x$S)
  }
  
  mx = max(a)
  a <- ifelse(a < zlim[1], zlim[1], a)
  a <- ifelse(a > zlim[2], zlim[2], a)
  
  image(x$t, x$f, t(a), zlim=zlim, col=col, xlab=xlab, ylab=ylab,...)
}
