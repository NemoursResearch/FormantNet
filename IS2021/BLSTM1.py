...
# # Code for IS2021 BLSTM1 model (RNN model with 1 bidirectional LSTM layer)
# 
# The Python code (run under TensorFlow 2.3) that was used to train and evaluate the BLSTM1 model submitted to Interspeech 2021 is given below. The code is unaltered, except that (1) comments have been added, and (2) code used solely to evaluate the trained model on non-TIMIT data has been removed.
# 
...
# 
# ### Output:
# The output models and evaluation files are saved to a directory named expdir/mvt14_f6z1/ (where "mvt14" was the unique designation for this experiment, and "f6z1" indicates 6 formants and 1 zero). The model files are stored directly in this directory. A subdirectory, expdir/mvt14_f6z1/timit/, in which the output formant track files are stored, one for each input file. These are stored in a format (described below) that was designed to the specific interests of our laboratory, so scripts will be provided that were used to extract the frequencies for evaluation against the VTR-TIMIT database.
...
expname = "mvt14"    # A unique designation for the experiment -- used to name the output directories and files below
