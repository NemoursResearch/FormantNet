...
## This is an R script used to generate a report of the error of the formant estimates generated
## by a model, in comparison to the VTR-TIMIT F1-F3 reference values. It expects as input a single file
## in the format generated by get_report.sh; see that file for details. It can be run e.g. as so:

## Rscript ../timitscript.R timdb.mvt13.f6z1.txt | sed "s:\[1\] ::" > timreport.mvt13.f6z1.txt

## The script generates statistics, mostly MAEs (mean absolute errors) and RMSEs (root mean
## square errors) of various subsets of the data and arranges them into tables for output. These
...
## Note that this script does not perform any binomial smoothing on the estimated frequencies
## prior to analysis -- nor do the Python scripts used to generate the original data, or
## get_report.sh. See timitscript2.sh for generating a similar report on smoothed data.
...
##The remaining name is appended to all output lines.
...
##Generate report for the given model, using the first 3 formants
...
## configuration is found, a second report is generated for the amended model. The lines for the
## new report will be appended with a suffix indicating the new formants used, e.g. "C134"
## would indicate that formants 1, 3, and 4 were used instead of 1-3.
